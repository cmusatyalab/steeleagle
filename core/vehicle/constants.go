...
// VideoStreamingType determines the type of video streaming that the vehicle uses.
// RTSP will forward an existing RTSP stream from the driver. Frames will encode
// individually sent frames into an RTSP stream.
...
// VideoResolution determines the resolution at which the stream is requested from the
// driver, and the resolution that is transmitted if there is sufficient bandwidth.
...
// ServerListenerName is the listener name for the server listener displayed in logs.
