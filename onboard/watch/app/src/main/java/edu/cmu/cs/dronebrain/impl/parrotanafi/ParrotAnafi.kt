...
class ParrotAnafi(sdk: ManagedGroundSdk) : DroneItf {
    ...
    override fun startStreaming(resolution: Int?) {
        ...
        Log.d(TAG, "Streaming thread starting...")
                ...
                if (decodeCounter % skip == 0) {
                    ...
                    for (i in 0 until decode) {
                        try {
                            ...
                            wrapper!!.grab()
                            Log.d(TAG, "Grabbed a frame, took " + (System.currentTimeMillis() - start) + "ms")
    ...
    override fun calculateOffsets(pixel_x: Int, pixel_y: Int, leash: Double): ArrayList<Double> {
        ...
        Log.d("Tracking", "Yaw angle: " + target_yaw_angle + ", Gimbal pitch angle: " + target_pitch_angle)
    ...
    fun executePCMD(gimbal_pitch: Double?, drone_yaw: Double, drone_pitch: Double, drone_roll: Double) {
        ...
        if (gpitch != null) setGimbalPose(0.0, ((-1 * gpitch / 2.5) + getGimbalPitch()).coerceIn(-90.0, 90.0), 0.0)
    ...
    override fun trackTarget(box: Vector<Double>?, leash: Double) {
        ...
        if (box != null) {
            ...
            var pixel_x = ((((box[3] - box[1]) / 2.0) + box[1])).roundToInt()
            var pixel_y = ((1 - (((box[2] - box[0]) / 2.0) + box[0]))).roundToInt()
            // Get offsets
    ...
    override fun lookAtTarget(box: Vector<Double>?) {
        ...
        if (box != null) {
            Log.d("Tracking", "Got box for target class!")
            // Find the pixel center
            Log.d("Tracking", "Resolution: " + resolution_x + ", " + resolution_y)
            Log.d("Tracking", "Bounding box: " + box[0] + ", " + box[1] + ", " + box[2] + ", " + box[3])
            var pixel_x = (((box[3] - box[1]) / 2.0) + box[1]).roundToInt()
            var pixel_y = (((box[2] - box[0]) / 2.0) + box[0]).roundToInt()
            // Get offsets
