# Interface imports
from base.IDroneDevice import IDroneDevice, TelemetryFrame, \
        ImageFrame, Response
from base.DataTypes import Attitude, Velocity, RelativePosition, \
...
from olympe.messages.ardrone3.PilotingSettingsState \
        import MaxTiltChanged
from olympe.messages.ardrone3.SpeedSettingsState \
        import MaxVerticalSpeedChanged, MaxRotationSpeedChanged
from olympe.messages.ardrone3.PilotingState import AttitudeChanged, \
        GpsLocationChanged, AltitudeChanged, FlyingStateChanged, \
        SpeedChanged
from olympe.messages.ardrone3.GPSState \
        import NumberOfSatelliteChanged
from olympe.messages.gimbal import set_target, attitude
from olympe.messages.wifi import rssi_changed
from olympe.messages.battery import capacity
from olympe.messages.common.CalibrationState \
        import MagnetoCalibrationRequiredState
...
class OlympeDrone(IDroneDevice):
    ...
    async def setGlobalPosition(self, , bearing):
        await self.switchModes(ParrotDrone.FlightMode.GUIDED)
        if bearing is None:
            self.drone(
    ...
    async def setGimbalAttitude(self, attitude):
    ...
    async def getVelocityNED(self):
        ...
        return {"north": NED["speedX"], "east": NED["speedY"], "down": NED["speedZ"]}

    async def getVelocityBody(self):
        NED = await self.getVelocityNED()
        vec = np.array([NED["north"], NED["east"]], dtype=float)
