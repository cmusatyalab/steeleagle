...
def command_socket():
    ...
    setup_zmq_socket(command_socket, "cloudlet.swarm_controller", SocketOperation.BIND)
...
async def kernel(mock_services, command_socket):
    ...
    running = []
    subps = [["python", "kernel/main.py", "--test"]]
    for subp in subps:
        s = subprocess.Popen(subp)
        running.append(s)
        await asyncio.sleep(1)
    # Wait for needed services
    required = [test_proto.ServiceType.CORE_SERVICES]
    await wait_for_services(required, command_socket)
    yield
    for subp in running:
        subp.terminate()
...
async def imagery():
    ...
    setup_zmq_socket(imagery_socket, "internal.streams.imagery", SocketOperation.BIND)
...
async def results():
    ...
    setup_zmq_socket(result_socket, "internal.streams.results", SocketOperation.CONNECT)
