    ...
    def emit(self, record):
        try:
            ...
            request = LogRequest(
                ...
                topic=record.name,
                log=LogMessage(type=log_type, msg=record.getMessage()),
            )
...
def setup_logging():
    ...
    if query_config(
        "logging.generate_flight_log"
    ):  # Only send to the log service if it's running
        flight_log_handler = FlightLogHandler(
