...
async def main(args):
    ...
    command_socket.setsockopt(
        zmq.IDENTITY, query_config("vehicle.name").encode("utf-8")
    )
    setup_zmq_socket(
    ...
    report_service_pb2_grpc.add_ReportServicer_to_server(
        ReportService(command_socket), server
    )
    compute_service_pb2_grpc.add_ComputeServicer_to_server(
        ComputeService(stream_handler), server
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Runs core services and handles permissions."
    )
    parser.add_argument(
        "--test",
        action="store_true",
        help="Report when core services are ready for testing",
    )
    parser.add_argument(
        "--startup",
        nargs="+",
        default=[],
        help="List of startup commands to run before entering __BASE__ law",
    )
