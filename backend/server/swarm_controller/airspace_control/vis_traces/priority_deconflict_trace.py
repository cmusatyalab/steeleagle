...
async def run_priority_trace():
    """Execute trace demonstrating priority-based deconfliction."""
    ...
    print(f"\n{'='*60}")
    print(f"PRIORITY DECONFLICTION SCENARIO")
    print(f"{'='*60}")
    ...
    engine.set_priority(drone_id=1, new_priority=5)  # Higher priority
    engine.set_priority(drone_id=2, new_priority=3)  # Lower priority
    
    print("\nDrone priorities:")
    print("  Drone 1: Priority 5 (HIGHER)")
    print("  Drone 2: Priority 3 (LOWER)")
    
    # Define paths - both drones converge on the same central region
    drone_paths = {
        1: [
            (37.7775, -122.4175, 50),   # Start: NE, low
            (37.7750, -122.4150, 50),   # Destination: Center, low (CONFLICT POINT)
        ],
        2: [
    ...
    print("\nFlight paths (both target the same destination):")
    for drone_id, path in drone_paths.items():
        ...
        for i, pos in enumerate(path, 1):
            marker = " <- CONFLICT ZONE" if i == 2 else ""
            print(f"    {i}. ({pos[0]:.4f}, {pos[1]:.4f}, {pos[2]:.0f}m){marker}")
    
    print(f"\n{'='*60}")
    print(f"Starting concurrent flights...")
    print(f"Both drones will attempt to reserve the same region")
    print(f"{'='*60}\n")
    ...
    print(f"\n{'='*60}")
    print(f"Priority system successfully prevented conflict")
    print(f"Higher priority drone secured the contested region")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("PRIORITY DECONFLICTION TRACE")
    print("Demonstrating priority-based conflict resolution")
    print("="*60 + "\n")
