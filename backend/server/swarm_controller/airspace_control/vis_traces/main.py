...
def test_drone_operations(engine: AirspaceControlEngine):
    ...
    logger.info(
        f"High priority drone 1002 override attempt: {'SUCCESS' if success else 'FAILED'}"
    )
    ...
    logger.info(
        f"Unauthorized occupancy attempt: {'BLOCKED' if not success else 'ALLOWED'}"
    )
    ...
    logger.info(
        f"Reservation in no-fly zone: {'BLOCKED' if not success else 'ALLOWED'}"
    )
