...
class SwarmController(RemoteServicer):
    ...
    async def CompileMission(self, compile_request, context):
        ...
        response = CompileMissionResponse(
            compiled_dsl_content=mission_json_text, response=generate_response(2)
        )
        yield response
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    continue
                logger.info(
                    f"Received response: {response}, seq_num: {response.sequence_number}"
                )
...
async def main():
    ...
    logger.info(
        f"Listening on tcp//*:{args.commander_port} for commander connections..."
    )
    await server.start()
    try:
