...
def cross(
    base_corner: Coordinate, end_corner: Coordinate, ref_point: Coordinate
) -> float:
    return (
        (ref_point.long - base_corner.long) * (end_corner.lat - base_corner.lat)
    ) - ((ref_point.lat - base_corner.lat) * (end_corner.long - base_corner.long))
...
def haversine(origin: Coordinate, ref_point: Coordinate) -> tuple[float, float]:
    ...
    azimuth = (
        360.0 + np.rad2deg(np.atan2(y, x))
    ) % 360  # TODO Verify that this is not offset 90 degrees from navigational north
...
def scale_matrix(
    target_matrix: np.ndarray, x_factor: float, y_factor: float, z_factor: float
) -> np.ndarray:
    scale_mat = np.array(
        [x_factor, 0, 0, 0], [0, y_factor, 0, 0], [0, 0, z_factor, 0], [0, 0, 0, 1]
    )
    return scale_mat @ target_matrix


def translate_matrix(
    target_matrix: np.ndarray, x_trans: float, y_trans: float, z_trans: float
) -> np.ndarray:
    trans_mat = np.array(
        [1, 0, 0, x_trans], [0, 1, 0, y_trans], [0, 0, 1, z_trans], [0, 0, 0, 1]
    )
