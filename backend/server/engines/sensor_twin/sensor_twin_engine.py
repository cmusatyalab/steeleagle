...
class SensorTwinEngine:
    ...
    def parse_init_file(self, init_file):
        ...
        if fail_flag:
            logger.error(
                "Failed init due to incorrectly formatted origin object in init file..."
            )
        drones = data["drones"]
        ...
        for items in drones:
            if not self.validate_drone_inputs(drones):
                logger.error(
                    "Failed init due to incorrectly formatted drone objects in init file..."
                )
                fail_flag = True
        label_list = []
        for items in objects:
            ...
            if not self.validate_object_inputs(objects):
                logger.error(
                    "Failed init due to incorrectly formatted detection objects in init file..."
                )
                fail_flag = True
        if fail_flag:
            return False
    ...
    def parse_origin(self, json_origin):
        ...
        """
        if (
            "lat" not in json_origin
            or "lon" not in json_origin
            or "alt" not in json_origin
        ):
            return (None, True)
    ...
    def validate_drone_inputs(self, json_drone_list):
        ...
        for items in json_drone_list:
            ...
            missing_fov = 0
            fail_flag = False
            if "drone_name" not in items or type(items["drone_name"]) is not str:
                logger.error(
                    f"Invalid drone_name during drone validation. Expected name in string form, received {type(items['drone_name'])}"
                )
                fail_flag = True
            if "x_offset" not in items or (
                type(items["x_offset"]) is not float
                and type(items["x_offset"]) is not int
            ):
                logger.error(
                    f"Invalid x_offset during drone validation. Expected int or float, received {type(items['x_offset'])}"
                )
                fail_flag = True
            if "y_offset" not in items or (
                type(items["y_offset"]) is not float
                and type(items["y_offset"]) is not int
            ):
                logger.error(
                    f"Invalid y_offset during drone validation. Expected int or float, received {type(items['y_offset'])}"
                )
                fail_flag = True
            if "z_offset" not in items or (
                type(items["z_offset"]) is not float
                and type(items["z_offset"]) is not int
            ):
                logger.error(
                    f"Invalid z_offset during drone validation. Expected int or float, received {type(items['z_offset'])}"
                )
                fail_flag = True
            if "orientation" not in items or type(items["orientation"]) is not int:
                logger.error(
                    f"Invalid orientation during drone validation. Expected int, received {type(items['orientation'])}"
                )
                fail_flag = True
            if (
                "gimbal_rotation" not in items
                or type(items["gimbal_rotation"]) is not int
            ):
                logger.error(
                    f"Invalid gimbal rotation during drone validation. Expected int, received {type(items['gimbal_rotation'])}"
                )
                fail_flag = True
            if "hfov" not in items or (
                type(items["hfov"]) is not int and type(items["hfov"]) is not float
            ):
                logger.warning(f"No HFOV provided for drone_{counter}...")
                missing_fov += 1
            if "vfov" not in items or (
                type(items["vfov"]) is not int and type(items["vfov"]) is not float
            ):
                logger.warning(f"No VFOV provided for drone_{counter}...")
                missing_fov += 1
            if "dfov" not in items or (
                type(items["dfov"]) is not int and type(items["dfov"]) is not float
            ):
                logger.warning(f"No DFOV provided for drone_{counter}...")
                missing_fov += 1
            if missing_fov > 1:
                logger.error(
                    "Invalid FOV specifications during drone validation. Expected 2 of 3 of HFOV, VFOV, DFOV..."
                )
                fail_flag = True
            if fail_flag:
    ...
    def validate_object_inputs(self, json_object_list):
        ...
        for items in json_object_list:
            fail_flag = False
            if "object_name" not in items or type(items["object_name"]) is not str:
                logger.error(
                    f"Invalid object name during detection object validation for object {counter}. Expected string, received {type(items['object_name'])}"
                )
                fail_flag = True
            if "class_label" not in items or type(items["class_label"]) is not str:
                logger.error(
                    f"Invalid class label during detection object validation for object {counter}. Expected string, received {type(items['class_label'])}"
                )
                fail_flag = True
            if "base_confidence" not in items or (
                type(items["base_confidence"]) is not float
                and type(items["base_confidence"]) is not int
            ):
                logger.error(
                    f"Invalid base confidence during detection object validation for object {counter}. Expected float, received {type(items['base_confidence'])}"
                )
                fail_flag = True
            if "falloff_start_distance" not in items or (
                type(items["falloff_start_distance"]) is not float
                and type(items["falloff_start_distance"]) is not int
            ):
                logger.error(
                    f"Invalid falloff starting distance during detection object validation for object {counter}. Expected float or int, received {type(items['falloff_start_distance'])}"
                )
                fail_flag = True
            if "object_length" not in items or (
                type(items["object_length"]) is not float
                and type(items["object_length"]) is not int
            ):
                logger.error(
                    f"Invalid object length during detection object validation for object {counter}. Expected float or int, received {type(items['object_length'])}"
                )
                fail_flag = True
            if "object_width" not in items or (
                type(items["object_width"]) is not float
                and type(items["object_width"]) is not int
            ):
                logger.error(
                    f"Invalid object width during detection object validation for object {counter}. Expected float or int, received {type(items['object_width'])}"
                )
                fail_flag = True
            if "object_height" not in items or (
                type(items["object_height"]) is not float
                and type(items["object_height"]) is not int
            ):
                logger.error(
                    f"Invalid object height during detection object validation for object {counter}. Expected float or int, received {type(items['object_height'])}"
                )
                fail_flag = True
            if "x_offset" not in items or (
                type(items["x_offset"]) is not float
                ...
                logger.error(
                    f"Invalid x offset during detection object validation for object {counter}. Expected float or int, received {type(items['x_offset'])}"
                )
                fail_flag = True
            if "y_offset" not in items or (
                type(items["y_offset"]) is not float
                ...
                logger.error(
                    f"Invalid y offset during detection object validation for object {counter}. Expected float or int, received {type(items['y_offset'])}"
                )
                fail_flag = True
            if "z_offset" not in items or (
                type(items["z_offset"]) is not float
                ...
                logger.error(
                    f"Invalid z offset during detection object validation for object {counter}. Expected float or int, received {type(items['z_offset'])}"
                )
                fail_flag = True
            if fail_flag:
    ...
    def create_detection_object(self, raw_det_obj):
        if len(raw_det_obj) <= DETOBJ_FEATURE_COUNT:
            logger.error(
                f"Attempted to produce detection object from improperly formatted values. Expected {DETOBJ_FEATURE_COUNT}, raw object contained {len(raw_det_obj)}"
            )
            return None
    ...
    def create_viewport(self, raw_vp_dict, origin: Coordinate):
        vp = Viewport(
            ...
            Coordinate(
                raw_vp_dict["x_offset"],
                raw_vp_dict["y_offset"],
                raw_vp_dict["z_offset"],
            ),
        ...
        if vp.horizontal_fov is None:
            vp.derive_hfov()
        if vp.vertical_fov is None:
            vp.derive_vfov()
        if vp.diagonal_fov is None:
            vp.derive_dfov()
        vp.derive_aspect_ratio()

        if (
            vp.horizontal_fov is None
            or vp.vertical_fov is None
            or vp.diagonal_fov is None
        ):
            logger.error(
                f"Failed to specify at least two of three FOV widths. Received "
    ...
    def handle(self, input_frame):
        ...
        if len(results) > 0:
            gabriel_result = self.process_results(
                image_np, results, cpt_config, extras.telemetry, extras.drone_name
            )
    ...
    def update_viewport(self, drone_name: str, telemetry):
        if not telemetry.HasField("global_position"):
            logger.error(
                f"Telemetry packet received without global position field for {drone_name}"
            )
        loc = Coordinate(
        ...
        logger.debug(
            f"Set viewport for {drone_name} to global position {self.viewport_list[drone_name].get_global_position()} at time {time.time()}"
        )

    def update_detection_object(self, object_name: str, new_pos: Coordinate):
        self.detection_object_list[object_name].set_global_pos(
            new_pos.long, new_pos.lat, new_pos.alt
        )
        logger.debug(
            f"Set {object_name} to global position {self.detection_object_list[object_name].get_global_pos()} at time {time.time()}"
        )
    ...
    def process_image(self, image, image_path, drone_name):
        ...
        detections = [
            x.produce_detection(vp.get_global_position())
            for x in self.detection_object_list
            if vp.check_object(x)
        ]
        nd_image = np.asarray(image)
        if len(detections) == 0:
            # Returns a result containing the base image with no associated bounding boxes/detections
            return self.produce_simulated_result(
                nd_image, image_path, self.class_names, None
            )
        ...
        result = self.produce_simulated_result(
            nd_image, image_path, self.class_names, boxes
        )
        return result

    def detection_to_bounding_box(
        self, class_label: str, confidence: float, image_height: int, image_width: int
        ...
        box = ultralytics.engine.results.Boxes(
            [x1, y1, x2, y2, confidence, class_label], (image_height, image_width)
        )
        return box

    def produce_simulated_result(
        self,
        image: np.ndarray,
        path: str,
        class_names: dict,
        boxes: list[ultralytics.engine.results.Boxes] | None,
