...
class TerraSLAMEngine(cognitive_engine.Engine):
    ...
    def handle(self, input_frame, client_info):
        ...
        if (input_frame.WhichOneof("payload") != "any_payload"
                or not input_frame.any_payload.Is(telemetry.EncodedFrame.DESCRIPTOR)):
            status.code = gabriel_pb2.StatusCode.WRONG_INPUT_FORMAT
            status.message = "Expected an any_payload telemetry.EncodedFrame"
            return cognitive_engine.Result(status, None)

        vehicle_info = common_pb2.VehicleInfo()
        if client_info.Is(common_pb2.VehicleInfo.DESCRIPTOR):
        ...
        # Process the image
        slam_status = self.slam_client.process_image(frame.encoded_data)
        ...
        if slam_status == "success" and self.slam_client.latest_pose:
            ...
            # Store in Redis
            self.r.xadd(
                f"slam:{vehicle_info.vehicle_id}",
                {
                    "pose_x": 0.0,
                    "pose_y": 0.0,
                    "pose_z": 0.0,
        ...
        return cognitive_engine.Result(status, json.dumps(response))
