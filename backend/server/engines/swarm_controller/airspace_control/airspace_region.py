...
class AirspaceRegion:
    def __init__(
        ...
        logger.debug(f"AirspaceRegion created: lat={self.min_lat:.4f}-{self.max_lat:.4f}, "
    ...
    def contains(self, ref_lat: float, ref_lon: float, ref_alt: float) -> bool:
        ...
        if ref_alt < self.min_alt or ref_alt > self.max_alt:
            logger.debug(f"Point ({ref_lat:.4f}, {ref_lon:.4f}, {ref_alt:.0f}) outside altitude bounds of region {self.region_id}")
            return False
        ...
        for i in range(CORNER_COUNT):
            if not self.line_side_test(
                ...
                logger.debug(f"Point ({ref_lat:.4f}, {ref_lon:.4f}, {ref_alt:.0f}) outside lateral bounds of region {self.region_id}")
                return False
        logger.debug(f"Point ({ref_lat:.4f}, {ref_lon:.4f}, {ref_alt:.0f}) contained in region {self.region_id}")
        return True
    ...
    def add_lateral_neighbor(self, neighbor_id: str):
        if neighbor_id not in self.lateral_neighbors:
            ...
            logger.debug(f"Added lateral neighbor {neighbor_id} to region {self.region_id}")
    ...
    def add_upper_neighbor(self, neighbor_id: str):
        if neighbor_id not in self.upper_neighbors:
            ...
            logger.debug(f"Added upper neighbor {neighbor_id} to region {self.region_id}")
    ...
    def add_lower_neighbor(self, neighbor_id: str):
        if neighbor_id not in self.lower_neighbors:
            ...
            logger.debug(f"Added lower neighbor {neighbor_id} to region {self.region_id}")
    ...
    def remove_lateral_neighbor(self, neighbor_id: str):
        if neighbor_id in self.lateral_neighbors:
            ...
            logger.debug(f"Removed lateral neighbor {neighbor_id} from region {self.region_id}")
    ...
    def remove_upper_neighbor(self, neighbor_id: str):
        if neighbor_id in self.upper_neighbors:
            ...
            logger.debug(f"Removed upper neighbor {neighbor_id} from region {self.region_id}")
    ...
    def remove_lower_neighbor(self, neighbor_id: str):
        if neighbor_id in self.lower_neighbors:
            ...
            logger.debug(f"Removed lower neighbor {neighbor_id} from region {self.region_id}")
    ...
    def shares_side_with(self, other: 'AirspaceRegion') -> bool:
        ...
        shares_side = ((lat_adjacent and lon_overlap and alt_overlap) or
    ...
    def get_bounds(self) -> tuple[tuple[float, float], tuple[float, float], tuple[float, float]]:
        return (self.get_lat_bounds(), self.get_lon_bounds(), self.get_altitude_bounds())
    ...
    def is_available(self) -> bool:
        ...
        logger.debug(f"Region {self.region_id} availability check: {available} (status: {self.status.name})")
        return available
    ...
    def is_available_priority(self, req_priority: int) -> bool:
        if (
            ...
            logger.debug(f"Region {self.region_id} available for priority {req_priority} (status: {self.status.name})")
            return True
        ...
        if (
            ...
            if self.owner_priority is not None:
                ...
                logger.debug(f"Region {self.region_id} priority check: req={req_priority} vs current={self.owner_priority}, can_override={can_override}")
                return can_override
            return True
        
        logger.debug(f"Region {self.region_id} not available for priority {req_priority} (status: {self.status.name})")
        return False
    ...
    def update_status(self, new_status: RegionStatus):
        if self.status != new_status:
            ...
            logger.info(f"Region {self.region_id} status changed: {old_status} -> {new_status.name}")
            
            # Log critical status changes to security log
            if new_status in [RegionStatus.NOFLY, RegionStatus.RESTRICTED_AVAILABLE, RegionStatus.RESTRICTED_ALLOCATED, RegionStatus.RESTRICTED_OCCUPIED]:
                security_logger.warning(f"CRITICAL STATUS CHANGE: Region {self.region_id} -> {new_status.name}")
    ...
    def update_owner(self, new_owner: int | None, new_owner_priority: int | None):
        if self.owner != new_owner:
            ...
            logger.info(f"Region {self.region_id} owner changed: {old_owner} -> {new_owner} (priority: {new_owner_priority})")
            
            # Log ownership changes to security log
            if new_owner is not None:
                security_logger.info(f"OWNERSHIP GRANTED: Region {self.region_id} assigned to drone {new_owner} (priority: {new_owner_priority})")
            else:
                security_logger.info(f"OWNERSHIP RELEASED: Region {self.region_id} released from drone {old_owner}")
    ...
    def set_timeout(self, length: float):
        ...
        logger.debug(f"Region {self.region_id} timeout set: {length}s for drone {self.owner}")

    def clear_timeout(self):
        if self.timeout_len is not None:
            logger.debug(f"Region {self.region_id} timeout cleared for drone {self.owner}")
    ...
    def check_timeout(self) -> bool:
        ...
        if expired:
            ...
            logger.warning(f"Region {self.region_id} lease EXPIRED for drone {self.owner} (overdue by {-remaining_time:.1f}s)")
            security_logger.warning(f"LEASE EXPIRED: Region {self.region_id} lease expired for drone {self.owner}")
        else:
            remaining_time = (self.timeout_len + self.timeout_ref) - current_time
            logger.debug(f"Region {self.region_id} lease check: {remaining_time:.1f}s remaining for drone {self.owner}")
