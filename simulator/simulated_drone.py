...
class SimulatedDrone():
    ...
    async def state_loop(self):
        ...
        while self._active_connection:
            ...
            logger.debug(f"-----Current Time: {self.t_cycle_start - self.t_baseline} -----")
            if self._pending_action:
                ...
                # Start accelerating in proper direction only after previous task canceled
                    ...
                    self._pending_action = False
                    logger.debug(f"state_loop: Pending action changed to: {self._pending_action}")
    ...
    async def _cancel_current_action(self):
        ...
        self._zero_velocity()
        stop_result = await self._wait_for_condition(lambda: self.is_stopped(), timeout=TASK_TIMEOUT, interval=0.1)
        if not stop_result:
            logger.error("cancel_current_action: Failed to zero velocity prior to cancellation...")
        else:
            logger.info(f"{self.get_state("drone_id")} successfully stopped prior to canceling current action...")
        if self._check_target_active("gimbal"):
            self._set_gimbal_rotation(0, 0, 0)
    ...
    async def _register_pending_task(self):
        ...
        logger.debug(f"register_pending_task: Pending action set to {self._pending_action}")
        # Allow state loop time to process flags even if no active task currently exists
        await asyncio.sleep(1)
        result = await self._wait_for_condition(lambda: self.is_task_lock_open(), timeout=TASK_TIMEOUT, interval=0.1)
        if result:
    ...
    async def take_off(self):
        ...
        if not result:
            logger.warning("take_off: Pending task already queued, unable to register take off command...")
            return False
        else:
            logger.info("take_off: Task successfully registered, beginning procedure...")
        ...
        result = await self._wait_for_condition(lambda: self.is_takeoff_complete(), timeout=TASK_TIMEOUT, interval=0.1)
        ...
        if result:
            ...
            self._zero_velocity()
            await self._wait_for_condition(lambda: self.is_stopped(), timeout=TASK_TIMEOUT, interval=0.1)
            self.set_flight_state(common_protocol.FlightStatus.IDLE)
    ...
    async def land(self):
        ...
        result = await self._register_pending_task()
        if not result:
            logger.warning("land: Pending task already queued, unable to register take off command...")
            return False
        else:
        ...
        stop_result = await self._wait_for_condition(lambda: self.is_stopped(), timeout=TASK_TIMEOUT, interval=0.1)
        if not stop_result:
            logger.warning("land: Failed to stop prior to beginning descent procedure...")
            self._active_action = False
            return False
        else:
            logger.info(f"{self.get_state("drone_id")} successfully stopped prior to executing descent procedure...")

        current_position = self.get_current_position()
        self._set_position_target(current_position[0], current_position[1], 0)

        result = await self._wait_for_condition(lambda: self.is_landed(), timeout=TASK_TIMEOUT, interval=0.1)
        self._zero_velocity()
        stop_result = await self._wait_for_condition(lambda: self.is_stopped(), timeout=TASK_TIMEOUT, interval=0.1)
    ...
    async def move_to(self, lat, lon, altitude, heading_mode, bearing):
        ...
        result = await self._register_pending_task()
        if not result:
            logger.warning("move_to: Pending task already queued, unable to register take off command")
            return False
        else:
        ...
        if not stop_result:
            ...
            logger.info(f"{self.get_state("drone_id")} successfully stopped prior to orienting drone...")
        ...
        result = await self._wait_for_condition(lambda: self.is_drone_oriented(), timeout=TASK_TIMEOUT, interval=0.1)
        if not result:
        ...
        result = await self._wait_for_condition(lambda: self._check_position_reached(), timeout=None, interval=.1)
        current_position = self.get_current_position()
        ...
        if result:
            ...
            self._zero_velocity()
            await self._wait_for_condition(lambda: self.is_stopped(), timeout=TASK_TIMEOUT, interval=.1)
            self.set_flight_state(common_protocol.FlightStatus.HOVERING)
    ...
    async def extended_move_to(self, lat, lon, altitude, heading_mode, bearing, lateral_vel, up_vel, angular_vel):
        ...
        if (self.check_flight_state(common_protocol.FlightStatus.LANDED)
            or self.check_flight_state(common_protocol.FlightStatus.LANDING)
            or self.check_flight_state(common_protocol.FlightStatus.IDLE)):
            logger.warning(f"extended_move_to: {self.get_state("drone_id")} unable to execute move command"
                           "from ground. Take off first...")
            return False
        
        result = await self._register_pending_task()
        if not result:
            logger.warning("extended_move_to: Pending task already queued, unable to register take off command...")
            return False
        else:
            logger.info("extended_move_to: Successfully registered task, beginning procedure...")

        self._zero_velocity()
        stop_result = await self._wait_for_condition(lambda: self.is_stopped(), timeout=TASK_TIMEOUT, interval=0.1)
        if not stop_result:
            logger.warning("extended_move_to: Failed to stop prior to orienting drone...")
            self._active_action = False
            return False
        else:
        ...
        if result == None:
            logger.error("extended_move_to: Unable to partition lateral velocity value...")
            self._active_action = False
        ...
        result = await self._wait_for_condition(lambda: self._check_position_reached(), timeout=MOVE_TIMEOUT, interval=0.1)
        current_position = self.get_current_position()
    ...
    async def set_target(self, gimbal_id, control_mode, pitch, roll, yaw):
        ...
        result = await self._register_pending_task()
        if not result:
            logger.warning("set_target: Pending task already queued, unable to register take off command")
            return False
        else:
        ...
        if yaw != attitude["yaw"]:
            ...
            logger.info(f"set_target: Turning drone to {yaw} degrees before rotating gimbal...")
            result = await self._wait_for_condition(lambda: self.is_drone_oriented(), timeout=TASK_TIMEOUT, interval=0.1)
            if result:
                logger.info(f"set_target: Successfully turned drone to {yaw} degrees")
            else:
                attitude = self.get_state("attitude")
                logger.warning(f"{self.get_state("drone_id")} failed to rotate to target orientation of {yaw}...")
        ...
        result = await self._wait_for_condition(lambda: self.is_gimbal_oriented(), timeout=TASK_TIMEOUT, interval=0.1)
    ...
    def is_drone_oriented(self):
        attitude = self.get_state("attitude")
        if (abs(attitude["pitch"] - self._pose_target["pitch"]) > POSE_TOLERANCE
            or abs(attitude["roll"] - self._pose_target["roll"]) > POSE_TOLERANCE
            or abs(attitude["yaw"] - self._pose_target["yaw"]) > POSE_TOLERANCE):
            return False
        self.set_attitude(self._pose_target["pitch"], self._pose_target["roll"], self._pose_target["yaw"])
        self._set_drone_rotation(0, 0, 0)
        self._set_pose_target(None, None, None)
        return True
    ...
    def is_gimbal_oriented(self):
        ...
        if (abs(gimbal_pose["g_pitch"] - self._gimbal_target["g_pitch"]) > POSE_TOLERANCE
        or abs(gimbal_pose["g_roll"] - self._gimbal_target["g_roll"]) > POSE_TOLERANCE
        or abs(gimbal_pose["g_yaw"] - self._gimbal_target["g_yaw"]) > POSE_TOLERANCE):
            return False
        self.set_gimbal_pose(self._gimbal_target["g_pitch"], self._gimbal_target["g_roll"],
    ...
    def set_attitude(self, pitch: float, roll: float, yaw: float):
        ...
        logger.debug(f"set_attitude: Drone attitude set to pitch: {pitch}, roll: {roll}, yaw: {yaw}")
    ...
    def set_current_position(self, new_lat: float, new_lon: float, new_alt: float):
        ...
        logger.debug(f"set_current_position: Current position set to ({new_lat}, {new_lon}, {new_alt})")
    ...
    def set_gimbal_pose(self, pitch: float, roll: float, yaw: float):
        ...
        logger.debug(f"set_gimbal_pose: Gimbal pose set to g_pitch: {pitch}, g_roll: {roll}, g_yaw: {yaw}")
    ...
    def set_velocity(self, vel_x: float, vel_y: float, vel_z: float):
        ...
        logger.debug(f"set_velocity: Drone velocity set to ("
    ...
    def _calculate_acceleration_direction(self):
        ...
        if velocity == None:
            logger.error("_calculate_acceleration_direction: Velocity state not initialized...")
            return
        if (self._position_target["lat"] == None
            or self._position_target["lon"] == None
            or self._position_target["alt"] == None):
            logger.error("_calculate_acceleration_direction: Position target not set...")
        ...
        if (current_position[0] == None
            ...
            logger.error("_calculate_acceleration_direction: Current position not set...")
            ...
            return
        if (velocity["speedX"] == None
            or velocity["speedY"] == None
            or velocity["speedZ"] == None):
            logger.error("_calculate_acceleration_direction: Velocity component(s) not set...")
    ...
    def _check_braking_thresholds(self):
        ...
        if acceleration == None or velocity == None:
            logger.error("_check_braking_thresholds: Failed to retrieve drone velocity and acceleration")
            logger.error(f"Velocity: {velocity}")
            logger.error(f"Acceleration: {acceleration}")
            return
        logger.debug(f"_check_braking_thresholds: Acceleration before check: "
    ...
    def _check_drone_pose_reached(self):
        ...
        if (pose == None
            ...
            or pose["roll"] == None
            or pose["yaw"] == None):
            logger.error("_check_drone_pose_reached: Failed to retrieve current drone pose...")
        ...
        if (self._pose_target["pitch"] == None
            ...
            return
        if (abs(self._pose_target["pitch"] - pose["pitch"]) <= POSE_TOLERANCE
    ...
    def _check_gimbal_pose_reached(self):
        ...
        if (g_pose == None
            ...
            logger.error("_check_gimbal_pose_reached: Failed to retrieve current gimbal pose...")
        ...
        if (self._gimbal_target["g_pitch"] == None
            ...
            logger.error("_check_gimbal_pose_reached: Gimbal pose target not properly set...")
        ...
        if (abs(self._gimbal_target["g_pitch"] - g_pose["g_pitch"]) <= POSE_TOLERANCE
            ...
            self.set_gimbal_pose(self._gimbal_target["g_pitch"], 
                                 self._gimbal_target["g_roll"], self._gimbal_target["g_yaw"])
            self._set_gimbal_rotation(0, 0, 0)
        return
    ...
    def _check_position_reached(self):
        ...
        if (abs(current_position[0] - self._position_target["lat"]) <= DEG_MATCH_TOLERANCE
            ...
            self._set_position_target(None, None, None)
                ...
                #logger.info("_check_position_reached: Zeroing velocity due to position match...")
                #self._zero_velocity()
            self._position_flag = True
            logger.info("_check_position_reached: Clearing position target due to position match...")
            return True
        return False
    ...
    def _check_velocity_reached(self):
        ...
        if acceleration == None or velocity == None:
            logger.error("_check_velocity_reached: Failed to retrieve drone velocity and acceleration")
            logger.error(f"Velocity: {velocity}")
            logger.error(f"Acceleration: {acceleration}")
            return
    ...
    def _set_drone_rotation(self, pitch_rate: float, roll_rate: float, yaw_rate: float):
        ...
        logger.debug(f"_set_drone_rotation: Pose rotation rates set to "
    ...
    def _set_gimbal_rotation(self, g_pitch_rate: float, g_roll_rate: float, g_yaw_rate: float):
        ...
        logger.debug(f"_set_gimbal_rotation: Gimbal rotation rates set to "
    ...
    def _set_gimbal_target(self, new_g_pitch: Optional[float], new_g_roll: Optional[float], new_g_yaw: Optional[float]):
        ...
        logger.debug(f"_set_gimbal_target: Gimbal pose target set to: "
    ...
    def _set_pose_target(self, new_pitch: Optional[float], new_roll: Optional[float], new_yaw: Optional[float]):
        ...
        logger.debug(f"Pose target set to: "
                     f"\n     Pitch: {target_pitch}"
                     f"\n     Roll: {target_roll}"
                     f"\n     Yaw: {target_yaw}")
    ...
    def _set_position_target(self, pos_x: Optional[float], pos_y: Optional[float], pos_z: Optional[float]):
        ...
        logger.debug(f"_set_position_target: Position target set to ({pos_x}, {pos_y}, {pos_z})")
    ...
    def _update_drone_pose(self, dt):
        ...
        if update_rate_flag:
            logger.debug(f"_update_drone_pose: Updating drone rotation rates...")
    ...
    def _update_gimbal_pose(self, dt):
        ...
        logger.debug(f"_update_gimbal_pose: Gimbal pose set to {g_pitch}, {g_roll}, {g_yaw}")

        if update_rate_flag:
            logger.debug(f"_update_gimbal_pose: Updating gimbal rotation rates...")
    ...
    def partition_lateral_velocities(self, lateral_vel) -> Optional[tuple[float, float]]:
        current_pos = self.get_current_position()
        if (current_pos[0] == None
            or current_pos[1] == None
            or self._position_target == None
            or self._position_target["lat"] == None
            or self._position_target["lon"] == None):
            logger.error("partition_lateral_velocities: Component value improperly set...")
            logger.error(f"Current lat: {current_pos[0]}, Current lon: {current_pos[1]}")
            logger.error(f"Target lat: {self._position_target["lat"]}, Target lon: {self._position_target["lon"]}")
            return None
        x_dist = self.haversine(current_pos[0], current_pos[1],
                                self._position_target["lat"], current_pos[1])
        y_dist = self.haversine(self._position_target["lat"], current_pos[1],
                                self._position_target["lat"], self._position_target["lon"])
