...
def derive_ar_test():
    try:
        ...
        if not t1:
            print(f"derive_ar_test: Failed to calculate aspect ratio from HFOV and VFOV. Expected" 
...
def mag_to_ground_test():
    try:
        ...
        if not t1:
            print(f"mag_to_ground_test: Failed to calculate magnitude to ground on 45 degree angle. "
                  f"Expected 7.071, resulted in {mag}")
        vp1.set_vertical_cant(30)
        mag = vp1.magnitude_to_ground(vp1.get_vertical_cant())
        t2 = (round(mag, 5) == 10)
        if not t2:
            print(f"mag_to_ground_test: Failed to calculate magnitude to ground on 30 degree angle. "
                  f"Expected 10, resulted in {mag}")
        vp1.set_vertical_cant(0.1)
        mag = vp1.magnitude_to_ground(vp1.get_vertical_cant())
        t3 = (round(mag, 5) == 2864.79043)
        if not t3:
            print(f"mag_to_ground_test: Failed to calculate magnitude to ground on .1 degree angle. "
...
def centerpoint_test():
    try:
        ...
        t1 = (cp == res1)
        if not t1:
            print(f"centerpoint_test: Failed to calculate centerpoint at {vp1.get_vertical_cant()} cant and "
                  f"{vp1.get_view_orientation()}. Expected {res1}, resulted in {cp}")
        vp1.set_view_orientation(45)
        cp = vp1.get_center_point()
        res2 = Coordinate(2 + round((5 * np.sqrt(2) / 2), 5), 2 + round((5 * np.sqrt(2) / 2), 5), 0)
        t2 = (cp == res2)
        if not t2:
            print(f"centerpoint_test: Failed to calculate centerpoint at {vp1.get_vertical_cant()} cant and "
...
def top_values_test():
    try:
        ...
        if not t1:
            print(f"top_values_test: Failed to calculate top intercept and ground length. Expected "
                  f"{res1} given mag {rlen1}, resulted in {tp} given mag {tlen}")
        vp1.set_view_orientation(45)
        tp, tlen = vp1.get_top_values()
        rlen2 = rlen1
        res2 = Coordinate(round(np.cos(np.deg2rad(45)) * rlen2 + 2, 3), round(np.sin(np.deg2rad(45)) * rlen2 + 2, 3), 0)
...
def bottom_values_test():
    try:
        ...
        if not t1:
            print(f"bottom_values_test: Failed to calculate bottom intercept and ground length. Expected "
        ...
        if not t2:
            print(f"bottom_values_test: Failed to calculate top intercept and ground length. Expected "
...
def lateral_point_test():
    try:
        ...
        t1 = (top_corners[0] == Coordinate(-4.19066, 12.72253, 0) and top_corners[1] == Coordinate(8.19066, 12.72253, 0))
        t2 = (bottom_corners[0] == Coordinate(0.65389, 4.33154, 0) and bottom_corners[1] == Coordinate(3.34611, 4.33154, 0))
        if not t1:
            print(f"lateral_point_test: Failed to derive lateral bounding box top corners. Expected "
                  f"{Coordinate(-4.191, 12.72253, 0)} and {Coordinate(8.191, 12.72253, 0)}, resulted in "
                  f"{top_corners[0]} and {top_corners[1]}")
        if not t2:
            print(f"lateral_point_test: Failed to derive lateral bounding box bottom corners. Expected "
        ...
        top_corners = vp1.get_lateral_points(tv[0], tv[1])
        bottom_corners = vp1.get_lateral_points(bv[0], bv[1])
        t3 = (top_corners[0] == Coordinate(7.55040, 11.61356, 0) and top_corners[1] == Coordinate(11.61356, 7.55040, 0))
        t4 = (bottom_corners[0] == Coordinate(3.20690, 4.09040, 0) and bottom_corners[1] == Coordinate(4.09040, 3.20690, 0))
        if not t3:
            print(f"lateral_point_test: Failed to derive lateral bounding box top corners. Expected "
...
def find_corners_test():
    try:
        ...
        rc = ([Coordinate(-4.19066, 12.72253, 0), Coordinate(0.65389, 4.33154, 0),
               Coordinate(3.34611, 4.33154, 0), Coordinate(8.19066, 12.72253, 0)])
        t1 = True
        for i in range(4):
            if corners[i] != rc[i]:
                t1 = False
        if not t1:
            print(f"find_corners_test: Failed to derive bounding box corners. Expected {rc}, resulted "
                  f"in {corners}")
        tc.set_global_pos(0,0,0)
        return t1
    except Exception as e:
...
def update_bounding_box_test():
    try:
        ...
        vp1.set_hfov(60)
        vp1.set_vfov(40)
        t1 = (vp1.bounding_box.corners == None and vp1.bounding_box.orientation == 0)
        if not t1:
            print(f"update_bounding_box_test: Failed to properly initialize bounding box")
        vp1.update_bounding_box()
        rc = [Coordinate(-4.19066, 12.72253, 0), Coordinate(0.65389, 4.33154, 0),
               Coordinate(3.34611, 4.33154, 0), Coordinate(8.19066, 12.72253, 0)]
        t2 = (vp1.bounding_box.corners == rc and vp1.bounding_box.orientation == 0)
        if not t2:
            print(f"update_bounding_box_test: Failed to update viewport bounding box. Expected {rc}, "
